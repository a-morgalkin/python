...
def change_contact():
    ...
    if phonebook.get(contact_id):
        ...
        new_name = input('>>> Введите новое имя (enter оставить)> '
                        ) or phonebook[contact_id][NAME]
        new_lastname = input('>>> Введите новую фамилию имя (enter оставить)> '
                        ) or phonebook[contact_id][LAST_NAME]
        new_phone = input('>>> Введите новый телефон (enter оставить) > '
                        ) or phonebook[contact_id][PHONE]
        new_title = input('>>> Введите новую должность (enter оставить) > '
...
def pb_open():
    ...
    if isfile(pb_file_name):
        with open(pb_file_name, 'r', encoding='utf-8') as pb_file:
            phonebook.clear()
            for i, line in enumerate(pb_file):
